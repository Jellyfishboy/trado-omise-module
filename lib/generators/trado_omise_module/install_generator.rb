module TradoOmiseModule
    module Generators
        class InstallGenerator < Rails::Generators::Base
            ...
            def copy_migration
                unless omise_migration_already_exists?
                    timestamp_number = Time.now.utc.strftime("%Y%m%d%H%M%S").to_i
                    copy_file "migration.rb", "db/migrate/#{timestamp_number}_add_omise_attributes.rb"
                end
            end
    ...
    #         end
            ...
            def omise_migration_already_exists?
                Dir.glob("#{File.join(destination_root, File.join("db", "migrate"))}/[0-9]*_*.rb").grep(/\d+_add_omise_attributes.rb$/).first
            end
        end
    end
end